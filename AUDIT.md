...
## 3. CURRENT FINDINGS — Snapshot 2026-06-12 (run: =audit)

### 🔴 FAIL — confirmed by grep (priority order)
| # | Pillar | Item | File:line | Role |
| --- | --- | --- | --- | --- |
| 1 | 3 | Container lệch — `.slider_cont` (padding:20px 80px, left:0) | components.css:40 | 🔵🟡 |
| 2 | 3 | Container lệch — `.hero-left` (padding:60px 80px, full-width) | components.css:1435 | 🔵🟡 |
| 3 | 4 | Sidenav không đóng bằng Escape — không có keydown handler | scripts.js | 🔵🟡 |
| 4 | 4 | `alt="invest"` generic, không mô tả | projects.html:119 | 🔵 |
| 5 | 2 | EmailJS CDN thiếu `integrity` SRI hash | contact.html:18 | 🔵 |
| 6 | 2 | Comment "Formspree honeypot" sai — thực tế dùng EmailJS | contact.html:124 | 🔵 |
| 7 | 2 | 404.html không tồn tại | (missing file) | 🔵🟢 |
| 8 | 1 | `<img>` thiếu `loading="lazy"` + `width`/`height` | projects.html:118 | 🔵 |
| 9 | 3 | Inline `style="font-family…font-size:28px"` | index.html:126 | 🔵 |
| 10 | 3 | Inline `style="font-size:25px"` *(new)* | index.html:150 | 🔵 |
| 11 | 1 | `<script src>` thiếu `defer` *(new — 5 file)* | about/contact/index/project-detail/projects.html | 🔵 |
| 12 | 3 | Inline link color trên breadcrumb *(new)* | project-detail.html:85 | 🔵 |

### ⚠️ SUSPECT — cần xác nhận có container wrapper không
| | Item | File:line |
|---|------|-----------|
...
- Focus trap trong sidenav khi mở
- Tab order hợp lý
- `--gray-text #848484` (3.74:1) — xác nhận context dùng: body text → FAIL; label/placeholder → có thể chấp nhận
...
- 3/5 trang dùng `.container` đúng (projects, project-detail, contact)
